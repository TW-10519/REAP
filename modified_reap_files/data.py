...
try:
    ...
    TokensPrompt = None  # vllm not needed for pruning, only eval
...
class LocalPromptsChatDataset(ChatDatasetProcessor):
    """Dataset for locally stored prompt text files."""
    ...
    for raw_line in text.splitlines():
        ...
        if not line:
            if in_prompt:
                current.append("")
            continue
        if line.lower().startswith("level") or line.lower().startswith("goal"):
...
def build_local_prompts_dataset(prompts_dir: str, prompts_glob: str = "*.txt") -> Dataset:
    base_dir = Path(prompts_dir).expanduser().resolve()
    if not base_dir.exists():
        raise FileNotFoundError(f"Local prompts directory not found: {base_dir}")

    entries: list[dict[str, any]] = []
    for prompt_file in sorted(base_dir.glob(prompts_glob)):
        if not prompt_file.is_file():
            continue
        content = prompt_file.read_text(encoding="utf-8", errors="ignore")
        prompts = _extract_numbered_prompts(content)
    ...
    if not entries:
        raise ValueError(
            f"No prompts found in {base_dir} with pattern '{prompts_glob}'."
        )
...
DATASET_REGISTRY: dict[str, BaseDatasetProcessor] = {
    ...
    "local-prompts": LocalPromptsChatDataset,
}
