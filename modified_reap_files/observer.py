    ...
    def _hook_factory(self, module: nn.Module, layer_number: int) -> callable:
        ...
        def _hook_fn(module, args, output):
            ...
            if self.hook_config.fused_experts:
                ...
                effective_top_k = int(top_k) if not isinstance(top_k, int) else top_k
                _, selected_experts = torch.topk(router_logits, effective_top_k, dim=-1)
                selected_experts = selected_experts.to(device)
                if not self.hook_config.skip_expert_replay:
                    router_indices = (
            ...
            if not self.hook_config.record_pruning_metrics_only:
                if activations is None:
                    raise ValueError(
                        "Activations are required for merging metrics but were not computed. "
                        "Disable merging metrics or enable expert replay."
                    )
            ...
            base_activations = flat_input if activations is None else None
            for i in range(num_experts):
                ...
                if activations is None:
                    ean_norm = torch.linalg.norm(base_activations[active_mask, :], dim=-1)
                else:
                ...
                # super experts
                if activations is None:
                    selected_activations = base_activations[active_mask, :]
                else:
...
class GptOssMoEObserverHookConfig(MoETransformerObserverConfig):
    module_class_name_to_hook_regex: Optional[str] = "GptOssMLP"  # ACTUAL class name from model inspection
    num_experts_attr_name: str = "experts.num_experts"
    top_k_attr_name: str = "router.top_k"
    fused_experts: bool = True  # GptOss uses fused gate_up_proj
    skip_expert_replay: bool = True  # GptOss experts require routing weights; avoid replay
