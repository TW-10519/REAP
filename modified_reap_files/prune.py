...
# Lazy import eval to avoid vllm CUDA issues on ARM64 systems
def _lazy_import_eval():
...
def prune(
    ...
    for layer in tqdm(observer_data, "Pruning layers..."):
        ...
        if not model_attrs["fused"]:
            ...
            if getattr(moe.router, "bias", None) is not None:
    ...
    # vLLM expects num_experts aliases for some MoE models (e.g., gpt_oss)
    if not hasattr(model.config, "num_experts"):
        model.config.num_experts = retained_experts
    else:
        model.config.num_experts = retained_experts
    if not hasattr(model.config, "n_experts"):
        model.config.n_experts = retained_experts
    else:
        model.config.n_experts = retained_experts
...
def main():
    ...
    if reap_args.do_eval:
        ...
        run_evaluate = _lazy_import_eval()
